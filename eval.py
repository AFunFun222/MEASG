...
import sys
from EvaluationMetrics.EvaluateUtil import eval_main
...
class Test(object):
    ...
    def __init__(self, model, vocab):
        ...
        self.model = models.Model(source_vocab_size=self.source_vocab_size, code_vocab_size=self.code_vocab_size,
    ...
    def test_iter(self):
        ...
        if config.save_test_outputs:
            try:
                ...
                logger.info('Test details file create failed')
        ...
        with torch.no_grad():
            ...
            for index_batch, batch in enumerate(p_bar):
                ...
                if config.save_test_outputs:
                    for index in range(len(candidates)):
                        ...
                        out_file.write('\n')
                        sample_id += 1
            DataUtil.save_pickle_data(config.plt_photo_dir, "total_references.pkl", total_references)
            DataUtil.save_pickle_data(config.plt_photo_dir, "total_candidates.pkl", total_candidates)
            # measure
            ...
            score_Rouge, score_Cider, score_Meteor, bleu4 = eval_main(total_references, total_candidates) 
            avg_scores = {'c_bleu': c_bleu, 's_bleu': s_blue_score, 'blue4': bleu4, 'meteor': score_Meteor, 'rouge': score_Rouge, 'cider': score_Cider}
            if out_file:
            ...
            for indices in sentences:
                for index in indices:  # indices is a list of length 1, only loops once
                    if index not in self.nl_vocab.index2word:  # current index is out of vocabulary
                        ...
                        try:
                            word = oovs[oov_index]
                            logger.info('Pointed OOV word: {}'.format(word))
                        except IndexError:
